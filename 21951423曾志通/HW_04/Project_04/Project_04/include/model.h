...
void Model::processNode(aiNode *node, const aiScene *scene) {
	...
	for (unsigned int i = 0; i < node->mNumMeshes; ++i) {
		// the node boject only contains indices to index the actual objects in the scene.
		// the scene ccontains all the data, node is just to keep stuff organized (like relations between nodes).
...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene) {
	...
	// now walk through each of the mesh's face (a face is a mesh its triangle) and retrieve the corresponding vertex indices.
