...
namespace CELL
{
    ...
    */
    ...
    tmat4x4<float> createRotate(tvec3<float> from, tvec3<float> dir, float angle)
    {
        float C =  cos(DEG2RAD(angle));
        float S =  sin(DEG2RAD(angle));
        float u = dir.x, v = dir.y, w = dir.z, a = from.x, b=from.y, c=from.z;

        tmat4x4<float> res;

        res[0][0] = u*u+(v*v+w*w)*C;
        res[0][1] = u*v*(1-C)-w*S;
        res[0][2] = u*w*(1-C)+v*S;
        res[0][3] = (a*(v*v+w*w)-u*(b*v+c*w))*(1-C)+(b*w-c*v)*S;
        res[1][0] = u*v*(1-C)+w*S;
        res[1][1] = v*v+(u*u+w*w)*C;
        res[1][2] = v*w*(1-C)-u*S;
        res[1][3] = (b*(u*u+w*w)-v*(a*u+c*w))*(1-C)+(c*u-a*w)*S;
        res[2][0] = u*w*(1-C)-v*S;
        res[2][1] = v*w*(1-C)+u*S;
        res[2][2] = w*w+(u*u+v*v)*C;
        res[2][3] = (c*(u*u+v*v)-w*(a*u+b*v))*(1-C)+(a*v-b*u)*S;
        res[3][0] = (0);
        res[3][1] = (0);
        res[3][2] = (0);
        res[3][3] = (1);
...
//        res[1][0] = u*v*(1-C)-w*S;
//        res[2][0] = u*w*(1-C)+v*S;
//        res[3][0] = (a*(v*v+w*w)-u*(b*v+c*w))*(1-C)+(b*w-c*v)*S;
//        res[0][1] = u*v*(1-C)+w*S;
//        res[1][1] = v*v+(u*u+w*w)*C;
//        res[2][1] = v*w*(1-C)-u*S;
//        res[3][1] = (b*(u*u+w*w)-v*(a*u+c*w))*(1-C)+(c*u-a*w)*S;
//        res[0][2] = u*w*(1-C)-v*S;
//        res[1][2] = v*w*(1-C)+u*S;
//        res[2][2] = w*w+(u*u+v*v)*C;
//        res[3][2] = (c*(u*u+v*v)-w*(a*u+b*v))*(1-C)+(a*v-b*u)*S;
//        res[0][3] = (0);
//        res[1][3] = (0);
//        res[2][3] = (0);
//        res[3][3] = (1);
