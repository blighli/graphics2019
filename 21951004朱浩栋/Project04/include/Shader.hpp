...
//    unsigned int VAO, VBO;
    ...
    void Draw(Shader shader)
    {
        for(int i=0;i<10;i++){
            if(i == 0){
                glUniform1i(shader._isTrans[i],2);
                CELL::matrix4 trans;trans[0].x = x; trans[0].y = 0; trans[0].z = z;
                glUniformMatrix4fv(shader._trans[i], 1, GL_FALSE, trans.data());
            }
            else if(i == 1){
                glUniform1i(shader._isTrans[i],1);
                CELL::matrix4 rot = CELL::createRotate(CELL::tvec3<float>(x0,0,z0),CELL::tvec3<float>(0,1,0),deg);
                glUniformMatrix4fv(shader._trans[i], 1, GL_FALSE, rot.data());
            }
            else glUniform1i(shader._isTrans[i],0);
        }
        glUniform1f(shader._roughness, r);
        glUniform1f(shader._metallic, m);
        glVertexAttribPointer(shader._position, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), vertices);
        glVertexAttribPointer(shader._normal, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), &(vertices[0].nx));
        glVertexAttribPointer(shader._color, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), &(vertices[0].r));
        glDrawArrays(GL_TRIANGLE_STRIP,0,Numb);
    }
};
...
public:
    ...
    void BeginContact(b2Contact* contact) {
        float A = contact->GetFixtureA()->GetFriction();
        float B = contact->GetFixtureB()->GetFriction();
        if( A==0.01f || B==0.01f) score+=1;
    }
